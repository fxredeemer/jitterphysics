    ...
    public partial class SoftBody : IBroadphaseEntity
    {
        ...
        public class Spring : Constraint
        {
            ...
            public override void Iterate()
            {
                ...
                var jv = JVector.Dot(body1.linearVelocity, jacobian[0]);
        ...
        public class Triangle : ISupportMappable
        {
            ...
            {
                var min = JVector.Dot(Owner.points[indices.I0].position, direction);
                var dot = JVector.Dot(Owner.points[indices.I1].position, direction);
                ...
                for (var e = 0; e < queryList.Count; e++)
                {
                    ...
                    if (!(t.VertexBody1 == points[i] || t.VertexBody2 == points[i] || t.VertexBody3 == points[i]))
                    {
                        if (XenoCollide.Detect(
                            ...
                            out var penetration))
                        {
                            var nearest = CollisionSystem.FindNearestTrianglePoint(this, queryList[e], point);
